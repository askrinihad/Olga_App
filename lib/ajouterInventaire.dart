...
class _ajouterInventaireState extends State<ajouterInventaire> {
  ...
  void initState() {
    ...
    _dateDebutController.text = DateFormat('dd-MM-yyyy HH:mm:ss').format(DateTime.now());
    _dateFinController.text = DateFormat('dd-MM-yyyy HH:mm:ss').format(DateTime.now());
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return CheckboxListTile(
              title: Text('${users[index]['prénom']} ${users[index]['nom']}'),
          ...
          setState(() {
            ...
            if (value == true) {
              ...
              selectedUserEmails.remove(users[index]['email']);
            }
          });
                      ...
                      showDialog(
                       ...
                       builder: (BuildContext context) {
                          return AlertDialog(
                           ...
                           content: Text(AppLocalizations.of(context)!.codeInventaireAjoute),
                            actions: [
                                ElevatedButton(
                                   onPressed: () {
...
Future<void> _showDateDebutTimePicker(BuildContext context) async {
  ...
  if (pickedDate != null) {
    ...
    if (pickedTime != null) {
      ...
      setState(() {
        _dateDebutController.text = selectedDateTime.toString(); // Adjust as needed
      });
    }
  }
}
...
Future<void> _showDateFinTimePicker(BuildContext context) async {
  ...
  if (pickedDate != null) {
    ...
    if (pickedTime != null) {
      ...
      setState(() {
        _dateFinController.text = selectedDateTime.toString(); // Adjust as needed
      });
    }
  }
}
//////////////////////////////////////
  void getUsersFromFirebase() async {
    ...
    setState(() {
      users = querySnapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList(); // Explicitly cast to Map<String, dynamic>
      selectedUsers = List<bool>.filled(users.length, false); // Initialize selection list
    });
  }
